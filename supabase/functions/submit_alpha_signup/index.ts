...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('Received signupData by Edge Function:', JSON.stringify(signupData, null, 2));
    console.log("Raw signup data received by Edge Function:", JSON.stringify(signupData, null, 2))

    // Validate required fields
    if (!signupData.email || !signupData.first_name || !signupData.last_name || !signupData.airline || !signupData.job_title || !signupData.crew_base) {
    ...
    console.log('Data just before Supabase count:', JSON.stringify(signupData, null, 2));
    ...
    const objectToInsert = {
      ...
      job_title: signupData.job_title, // Ensure this is from signupData
      crew_base: signupData.crew_base, // Added crew_base
      agreed_to_terms: signupData.agreed_to_terms,
      // signed_up_at can be handled by DB default or set here if needed
    };

    // Log the object before attempting insert
    console.log('Attempting to insert this object into Supabase alpha_signups:', JSON.stringify(objectToInsert, null, 2));

    // Attempt to insert the new signup
    const { data: insertData, error: insertError } = await supabaseClient
      .from('alpha_signups')
      .insert(objectToInsert) // Pass the constructed object directly
      .select()
      .single();
                ...
                <tr>
                  <td align="left" style="font-family: Arial, sans-serif; font-size: 16px; line-height: 24px; color: #333333; padding-bottom: 20px;">
                    ...
                    <ul style="list-style-type: none; padding-left: 0; margin-bottom: 20px;">
                      ...
                      ${signupData.crew_base ? `<li style="margin-bottom: 5px;"><strong>Crew Base:</strong> ${signupData.crew_base}</li>` : ''}
                    </ul>
                    <p style="margin: 0 0 15px 0;">You're all set for now! We'll be in touch soon with more details and instructions on how to get started with the SkyGuide Alpha.</p>
...
Airline: ${signupData.airline}
Job Title: ${signupData.job_title}
Crew Base: ${signupData.crew_base}

You're all set for now! We'll be in touch soon with more details and instructions on how to get started with the SkyGuide Alpha.
...
If you did not sign up for this list, please disregard this email.
      ...
      try {
        ...
        const userEmailSendResponse = await resend.emails.send({
        ...
        // Check if user email was sent successfully before proceeding
        if (userEmailSendResponse.data && userEmailSendResponse.data.id) {
          console.log('Confirmation email sent successfully to:', signupData.email, 'ID:', userEmailSendResponse.data.id);

          // Now, send notification email to admin
          const adminEmailTo = 'alpha@skyguide.site';
          const adminEmailSubject = 'New SkyGuide Alpha Signup';
          const adminEmailHtmlContent = `
            <h1>New Alpha Signup</h1>
            <p>A new user has signed up for the SkyGuide Alpha program:</p>
            <ul>
              <li><strong>Name:</strong> ${signupData.first_name} ${signupData.last_name}</li>
              <li><strong>Email:</strong> ${signupData.email}</li>
              ${signupData.airline ? `<li><strong>Airline:</strong> ${signupData.airline}</li>` : ''}
              ${signupData.job_title ? `<li><strong>Job Title:</strong> ${signupData.job_title}</li>` : ''}
            </ul>
          `;
          const adminEmailTextContent = `
            New Alpha Signup:
            Name: ${signupData.first_name} ${signupData.last_name}
            Email: ${signupData.email}
            ${signupData.airline ? `Airline: ${signupData.airline}` : ''}
            ${signupData.job_title ? `Job Title: ${signupData.job_title}` : ''}
          `;
          ...
          try {
            const adminEmailResponse = await resend.emails.send({
              ...
              html: adminEmailHtmlContent,
              text: adminEmailTextContent,
            });
            if (adminEmailResponse.data && adminEmailResponse.data.id) {
              console.log('Admin notification email sent successfully to:', adminEmailTo, 'ID:', adminEmailResponse.data.id);
        ...
        } else if (userEmailSendResponse.error) {
          // Log error if user confirmation email failed
          console.error('Error sending confirmation email to user:', userEmailSendResponse.error);
        }
      } catch (emailError) { // This outer catch handles errors from the userEmailSendResponse promise itself
        console.error('Failed to send confirmation email to user (exception):', emailError.message || emailError);
